module Appium
  module Ios
    class << self
      ...
      #
      #   ```ruby
      #    find_elements :class_chain, 'XCUIElementTypeWindow/XCUIElementTypeButton[3]' # select the third child button of the first child window element
      #    find_elements :class_chain, 'XCUIElementTypeWindow' # select all the children windows
      #    find_elements :class_chain, 'XCUIElementTypeWindow[2]/XCUIElementTypeAny[-2]' # select the second last child of the second child window
      #   ```
      def extended(_mod)
